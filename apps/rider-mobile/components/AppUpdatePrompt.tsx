  ...
  useEffect(() => {
    ...
    async function checkVersion() {
      try {
        ...
        const config = payload?.data?.mobileApps?.rider as
          | UpdateConfig
          | undefined;
        ...
        const message =
          ...
          "GoViet247 đã có phiên bản mới.";

        const openStore = () => {
          Linking.openURL(storeUrl).catch(() => {
            Alert.alert("Không mở được cửa hàng", "Vui lòng mở App Store hoặc Google Play và tìm GoViet247.");
          });
        };
